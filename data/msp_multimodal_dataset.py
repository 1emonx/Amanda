...
class mspmultimodaldataset(BaseDataset):
    ...
    def __init__(self, opt, set_name):
        ...
        config = json.load(open(os.path.join(pwd, 'config', f'{opt.corpus_name}at_config.json')))
    ...
    def __getitem__(self, index):
        ...
        # if torch.sum(torch.isnan(A_feat)) >= 1:
        #     print('AAAA', index, torch.sum(torch.isnan(A_feat)))
        # if torch.sum(torch.isnan(V_feat)) >= 1:
        #     print('VVVV', index, torch.sum(torch.isnan(V_feat)))
        A_feat[torch.isnan(A_feat)] = 0
        V_feat[torch.isnan(V_feat)] = 0
        # print('A shape: ', A_feat.shape, torch.max(A_feat), torch.min(A_feat))
        # print('V shape: ', V_feat.shape, torch.max(V_feat), torch.min(V_feat))
        # print('L shape: ', L_feat.shape, torch.max(L_feat), torch.min(L_feat))
        # print('index:', index)
        # if np.isnan(torch.max(A_feat)): 
        #     print(A_feat)
        #     print(torch.argmax(A_feat))
        #     nindex = torch.argmax(A_feat)
        #     row = int(nindex/768)
        #     col = nindex % 768
        #     print(row, col)
        #     print(A_feat[row, col])
